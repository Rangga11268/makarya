...
export function AdminDashboardPage() {
  ...
  const [students, setStudents] = useState([
    ...
    },
    {
      id: "4",
      name: "Siti Rahma",
      email: "siti.rahma@ubsi.ac.id",
      nim: "12216666",
      prodi: "Ilmu Komunikasi",
      campus: "UBSI Cengkareng",
      status: "PENDING",
      joinedAt: "2026-08-29",
    },
    {
    ...
    },
    {
      id: "7",
      name: "Nadia Safitri",
      email: "nadia@ubsi.ac.id",
      nim: "12214444",
      prodi: "Akuntansi",
      campus: "UBSI Kramat 98",
      status: "PENDING",
      joinedAt: "2026-08-30",
    },
    {
  ...
  const [payouts, setPayouts] = useState([
    ...
    {
      id: "PO-882",
      mhsName: "Adelia Putri",
      bank: "Bank Mandiri (157000982716)",
      amount: 400000,
      fee: 20000,
      net: 380000,
      date: "2026-08-30",
    ...
    {
      id: "PO-880",
      mhsName: "Fajar Pratama",
      bank: "GoPay / 081298761234",
      amount: 1200000,
      fee: 60000,
      net: 1140000,
      date: "2026-08-27",
  ...
  const handleVerifyStudent = (id) => {
    setStudents(
      students.map((s) => (s.id === id ? { ...s, status: "VERIFIED" } : s)),
    );
    addToast(
      "Identitas mahasiswa berhasil diverifikasi dan diberi lencana resmi.",
      "success",
    );
  };
  ...
  const handleApprovePayout = (id) => {
    ...
    addToast(
      `Pencairan dana ${id} berhasil diproses dan dilepaskan ke rekening bank.`,
      "success",
    );
  };
  ...
  const handleResolveDispute = async (keputusan) => {
    ...
    try {
      ...
      await disputeApi.resolve(selectedDispute.id, {
        ...
        catatan_admin:
          mediationNote ||
          "Sengketa diselesaikan melalui mediasi resmi administrator Makarya.",
      });
      addToast(
        `Sengketa berhasil diselesaikan dengan putusan: ${keputusan}`,
        "success",
      );
  ...
  const totalVolumeGmv =
    projects.reduce((acc, p) => acc + (parseFloat(p.budget_max) || 0), 0) +
    2450000;
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            ...
            <p className="text-xs sm:text-sm text-slate-300 font-normal max-w-xl">
              Audit kliring keuangan escrow, mediasi sengketa mahasiswa-UMKM,
              verifikasi kampus, dan analitik performa ekosistem.
            </p>
          </div>
        ...
        <div className="space-y-8 animate-in fade-in">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="p-6 space-y-4">
              <div className="flex items-center justify-between border-b border-border pb-3">
                <h3 className="text-sm font-bold text-dark-900">
                  Persebaran Talenta per Program Studi (UBSI)
                </h3>
            ...
            <Card className="p-6 space-y-4">
              <div className="flex items-center justify-between border-b border-border pb-3">
                <h3 className="text-sm font-bold text-dark-900">
                  Distribusi Kategori Kebutuhan UMKM
                </h3>
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          {(() => {
            const filteredPayouts = payouts.filter((po) => {
              const matchStatus =
                escrowStatus === "ALL" || po.status === escrowStatus;
            ...
            const totalEscrowPages =
              Math.ceil(filteredPayouts.length / escrowPerPage) || 1;
                    ...
                    <tbody className="divide-y divide-border">
                      {paginatedPayouts.length === 0 ? (
                        <tr>
                          ...
                          >
                            Tidak ada data pencairan yang sesuai dengan filter
                            pencarian.
                          </td>
                        </tr>
                      ) : (
                        paginatedPayouts.map((po) => (
                          ...
                          >
                            <td className="py-3.5 px-4 font-mono font-bold text-dark-900">
                              {po.id}
                            </td>
                            <td className="py-3.5 px-4 font-semibold text-dark-900">
                              {po.mhsName}
                            </td>
          ...
          {disputes.length === 0 ? (
            <Card className="p-12 text-center space-y-3">
              ...
              <h4 className="text-base font-bold text-dark-900">
                Ekosistem 100% Kondusif
              </h4>
              <p className="text-xs text-muted max-w-md mx-auto">
                Tidak ada sengketa proyek yang sedang terbuka saat ini. Seluruh
                transaksi berjalan harmonis.
              </p>
            </Card>
          ) : (
            <div className="grid grid-cols-1 gap-4">
              {disputes.map((d) => (
                <Card
                  key={d.id}
                  className="p-6 border-rose-200 bg-rose-50/20 space-y-4"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border pb-3">
                    <div className="flex items-center gap-2">
                      <Scale className="w-5 h-5 text-rose-600" />
                      <h4 className="text-sm font-bold text-dark-900">
                        Kasus Sengketa #{d.id.substring(0, 8)}
                      </h4>
                      <Badge
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="p-3 bg-canvas rounded-xl border border-border space-y-1">
                      ...
                      <span className="text-[10px] text-muted">
                        Tertahan di Rekening Penampung Escrow
                      </span>
                    </div>
                  </div>
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          {(() => {
            const filteredStudents = students.filter((mhs) => {
              const matchStatus =
                kycStatus === "ALL" || mhs.status === kycStatus;
            ...
            const totalKycPages =
              Math.ceil(filteredStudents.length / kycPerPage) || 1;
                      ...
                      {paginatedStudents.length === 0 ? (
                        ...
                        paginatedStudents.map((mhs) => (
                          <tr
                            ...
                            className="hover:bg-canvas/50 transition-colors"
                          >
                            <td className="py-3.5 px-4 font-bold text-dark-900">
                              {mhs.name}
                            </td>
                            <td className="py-3.5 px-4 font-mono text-brand-indigo font-medium">
                              {mhs.email}
                            </td>
                            <td className="py-3.5 px-4 font-mono text-muted">
                              {mhs.nim}
                            </td>
                            <td className="py-3.5 px-4 text-dark-900 font-semibold">
                              {mhs.prodi}
                            </td>
                            <td className="py-3.5 px-4 text-muted">
                              {mhs.campus}
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          {(() => {
            const filteredProjects = projects.filter((proj) => {
              const matchCategory =
                projCategory === "ALL" || proj.kategori === projCategory;
              const matchStatus =
                projStatus === "ALL" || proj.status === projStatus;
            ...
            const totalProjPages =
              Math.ceil(filteredProjects.length / projPerPage) || 1;
                      ...
                      {paginatedProjects.length === 0 ? (
                        ...
                        paginatedProjects.map((proj) => (
                          <tr
                            ...
                            className="hover:bg-canvas/50 transition-colors"
                          >
                            <td className="py-3.5 px-4 font-bold text-dark-900 max-w-xs truncate">
                              {proj.judul}
                            </td>
                            <td className="py-3.5 px-4">
                            ...
                            <td className="py-3.5 px-4 font-bold font-sans text-dark-900">
          ...
          <div className="space-y-5 text-xs text-dark-900 text-left">
            <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl space-y-1">
              <span className="font-bold text-rose-900 block">
                Klaim Pelapor ({selectedDispute.pelapor_role}):
              </span>
              <p className="text-rose-800 leading-relaxed italic">
            ...
            <div className="space-y-2 pt-2 border-t border-border">
              <span className="block font-bold text-dark-900 text-xs">
                Pilih Putusan Mediasi Admin:
              </span>
