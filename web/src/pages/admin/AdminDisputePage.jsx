...
export function AdminDisputePage() {
  ...
  const handleResolveSubmit = async (e) => {
    ...
    try {
      ...
      addToast(
        "Sengketa berhasil diputuskan dan dana escrow dialokasikan.",
        "success",
      );
      setModalOpen(false);
      fetchDisputes();
      ...
      setResolveError(
        err.response?.data?.detail || "Gagal memproses resolusi sengketa",
      );
    } finally {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 space-y-8 font-sans">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-muted font-sans mt-1">
          Mediasi sengketa antara UMKM dan Mahasiswa dengan pembagian dana
          escrow yang adil dan transparan.
        </p>
      </div>
        ...
        {loading ? (
          <div className="space-y-4">
            {[1, 2].map((n) => (
              <div
                key={n}
                className="h-32 bg-surface rounded-3xl border border-border animate-pulse"
              />
            ))}
          </div>
          ...
          <div className="space-y-4">
            {paginatedDisputes.map((d) => (
              <Card
                key={d.id}
                className="p-6 space-y-4 rounded-3xl border border-border bg-surface shadow-xs"
              >
        ...
        <form onSubmit={handleResolveSubmit} className="space-y-4">
          ...
          <TextArea
            ...
            onChange={(e) =>
              setResolveForm({
                ...resolveForm,
                keputusan_admin: e.target.value,
              })
            }
            required
          />
