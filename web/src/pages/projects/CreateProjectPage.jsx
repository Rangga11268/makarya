...
export function CreateProjectPage() {
  ...
  const runAiRequirementAnalyzer = (rawText, categoryId) => {
    ...
    setTimeout(() => {
      ...
      if (categoryId === "DESIGN") {
        ...
        skills = ["Logo & Branding", "Desain Kemasan (Packaging)", "Adobe Illustrator", "Desain Menu & Banner"];
        ...
        skills = ["Landing Page HTML/CSS", "WordPress / Web Builder", "Fullstack Web (React/Python/PHP)"];
        ...
        skills = ["UI/UX Website", "UI/UX Mobile App", "Wireframing & Prototyping"];
        ...
        skills = ["Entry Data Excel / Spreadsheet", "Pembukuan Keuangan Sederhana"];
      }
  ...
  const handleStep1Next = (e) => {
    ...
    if (formData.deskripsi_raw.trim().length < 15) {
      setErrorMessage("Ceritakan kebutuhan usaha Anda minimal 15 karakter agar AI dapat menganalisis kebutuhan.");
      return;
    }
  ...
  const handleAddDeliverable = () => {
    ...
    setFormData({
      ...formData,
      deliverables: [...formData.deliverables, formData.customDeliverableInput.trim()],
      customDeliverableInput: "",
    });
  };
  ...
  const handleStep3Next = (e) => {
    ...
    if (isNaN(budget) || budget < 50000 || budget > 2000000) {
      setErrorMessage("Budget maksimal proyek berkisar antara Rp 50.000 s/d Rp 2.000.000.");
      return;
    }
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const deliverablesText = formData.deliverables.length > 0 
      ...
      addToast("Selamat! Proyek berhasil diterbitkan ke pasar Makarya.", "success");
  ...
  const currentCatObj = categories.find((c) => c.id === formData.kategori) || categories[0];
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8 font-sans">
      ...
      <div className="bg-surface p-4 sm:p-5 rounded-2xl border border-border shadow-xs">
        <div className="grid grid-cols-4 gap-2 sm:gap-4">
          ...
          {/* Step 1 */}
          <div className={`flex items-center gap-2.5 ${currentStep >= 1 ? "text-dark-900 font-bold" : "text-muted"}`}>
            <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${
              currentStep === 1 
                ...
                : (currentStep > 1 ? "bg-emerald-600 text-white" : "bg-canvas text-muted border border-border")
            }`}>
              {currentStep > 1 ? <CheckCircle2 className="w-4 h-4" /> : "1"}
            </div>
            <div className="hidden sm:block text-left">
              <span className="text-[11px] uppercase tracking-wider block font-semibold text-muted">Langkah 1</span>
              <span className="text-xs truncate block">Konteks Usaha</span>
            </div>
          </div>

          {/* Step 2 */}
          <div className={`flex items-center gap-2.5 ${currentStep >= 2 ? "text-dark-900 font-bold" : "text-muted"}`}>
            <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${
              currentStep === 2 
                ? "bg-brand-indigo text-white shadow-brand" 
                : (currentStep > 2 ? "bg-emerald-600 text-white" : "bg-canvas text-muted border border-border")
            }`}>
              {currentStep > 2 ? <CheckCircle2 className="w-4 h-4" /> : "2"}
            </div>
            <div className="hidden sm:block text-left">
              <span className="text-[11px] uppercase tracking-wider block font-semibold text-muted">Langkah 2</span>
          ...
          {/* Step 3 */}
          <div className={`flex items-center gap-2.5 ${currentStep >= 3 ? "text-dark-900 font-bold" : "text-muted"}`}>
            <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${
              currentStep === 3 
                ? "bg-brand-indigo text-white shadow-brand" 
                : (currentStep > 3 ? "bg-emerald-600 text-white" : "bg-canvas text-muted border border-border")
            }`}>
              {currentStep > 3 ? <CheckCircle2 className="w-4 h-4" /> : "3"}
            </div>
            <div className="hidden sm:block text-left">
              <span className="text-[11px] uppercase tracking-wider block font-semibold text-muted">Langkah 3</span>
              <span className="text-xs truncate block">Budget & Waktu</span>
            </div>
          </div>

          {/* Step 4 */}
          <div className={`flex items-center gap-2.5 ${currentStep >= 4 ? "text-dark-900 font-bold" : "text-muted"}`}>
            <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all ${
              currentStep === 4 
                ? "bg-brand-indigo text-white shadow-brand" 
            ...
            <div className="hidden sm:block text-left">
              <span className="text-[11px] uppercase tracking-wider block font-semibold text-muted">Langkah 4</span>
        ...
        <Card className="p-6 sm:p-8 space-y-6 animate-in fade-in">
          <div className="border-b border-border pb-4 space-y-1">
            ...
            <p className="text-xs sm:text-sm text-muted">
              Pilih bidang keahlian dan ceritakan produk atau masalah yang ingin Anda selesaikan.
            </p>
          </div>
          ...
          <form onSubmit={handleStep1Next} className="space-y-6">
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <h4 className="text-xs font-bold text-dark-900">{cat.label}</h4>
                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${
                          formData.kategori === cat.id ? "border-brand-indigo bg-brand-indigo text-white" : "border-border"
                        }`}>
                          {formData.kategori === cat.id && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                        </div>
                      </div>
            ...
            <div className="flex items-center justify-end pt-4 border-t border-border">
              <Button variant="brand" size="lg" type="submit" className="font-bold shadow-brand">
      ...
      {currentStep === 2 && (
        <Card className="p-6 sm:p-8 space-y-6 animate-in fade-in">
          <div className="border-b border-border pb-4 space-y-1">
            ...
            <p className="text-xs sm:text-sm text-muted">
              AI telah memecah cerita kebutuhan Anda menjadi daftar deliverable teknis yang jelas untuk mahasiswa. Anda dapat menambah atau menyunting poin di bawah ini.
            </p>
          </div>
          ...
          {aiAnalyzing ? (
            <div className="py-12 text-center space-y-3">
              ...
              <h4 className="text-sm font-bold text-dark-900">AI sedang memproses kebutuhan usaha Anda...</h4>
              <p className="text-xs text-muted">Mengekstrak spesifikasi output deliverable & pemetaan keahlian digital.</p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="block text-xs font-semibold text-dark-900 uppercase tracking-wider">
                  Daftar Deliverable yang Wajib Diserahkan Mahasiswa ({formData.deliverables.length} Item)
                </label>
                ...
                <div className="flex items-center gap-2 pt-2">
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, customDeliverableInput: e.target.value })}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                  ...
                  />
                  <Button variant="secondary" size="md" onClick={handleAddDeliverable} className="text-xs font-bold shrink-0">
              ...
              <div className="p-4 rounded-2xl bg-brand-indigo-light/30 border border-brand-indigo/20 space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.recommendedSkills.map((skill, idx) => (
                    <Badge key={idx} variant="brand" className="text-xs py-1 px-3">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-border">
                ...
                <Button variant="brand" size="lg" onClick={() => setCurrentStep(3)} className="font-bold shadow-brand">
      ...
      {currentStep === 3 && (
        <Card className="p-6 sm:p-8 space-y-6 animate-in fade-in">
          <div className="border-b border-border pb-4 space-y-1">
            ...
            <p className="text-xs sm:text-sm text-muted">
              Sesuaikan nilai imbalan yang layak untuk mahasiswa. Platform membatasi pagu maksimal Rp 2.000.000 demi standar micro-freelancing.
            </p>
          </div>
          ...
          <form onSubmit={handleStep3Next} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, budget_max: parseInt(e.target.value, 10) })}
            ...
            <div className="p-4 rounded-2xl bg-canvas border border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <span className="text-xs text-muted block">Saldo Aktif Dompet Anda Saat Ini:</span>
              ...
              <div className="text-xs text-slate-600 max-w-sm">
                💡 <span className="font-semibold">Informasi Escrow:</span> Saldo Anda baru akan dikunci saat Anda menerima proposal mahasiswa tertentu.
              </div>
            </div>
        ...
        <div className="space-y-6 animate-in fade-in">
          <Card className="p-6 sm:p-8 space-y-6">
            <div className="border-b border-border pb-4 space-y-1">
              ...
              <p className="text-xs sm:text-sm text-muted">
                Periksa kembali ringkasan proyek sebelum diterbitkan ke ribuan talenta mahasiswa Makarya.
              </p>
            </div>
            ...
            <div className="max-w-md mx-auto">
              <ProjectCard
                project={{
                  ...
                  total_pelamar: 0,
                  umkm_profile: {
                    nama_usaha: user?.email ? user.email.split("@")[0].toUpperCase() : "Usaha UMKM Anda",
                    kota: "Indonesia",
                  },
                }}
              />
            ...
            <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-xs text-emerald-900 flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h5 className="font-bold">Komitmen Perlindungan Dana 100% Escrow</h5>
                <p className="leading-relaxed text-emerald-800">
                  Dengan menerbitkan proyek ini, Anda setuju bahwa dana honor akan ditahan di rekening penampung resmi Makarya saat menerima proposal, dan baru diteruskan ke mahasiswa setelah Anda memeriksa dan menyetujui hasil deliverable.
                </p>
              </div>
            </div>

            <div className="flex items-center justify-between pt-4 border-t border-border">
              <Button variant="secondary" size="md" onClick={() => setCurrentStep(3)} disabled={submitting}>
                <ArrowLeft className="w-4 h-4 mr-1.5" />
                Kembali & Edit
              </Button>
              <Button
